...
ggplot(data = penguins_data, aes(x = flipper_length_mm, fill = species)) +
  geom_histogram(alpha = 0.75) + #alpha changes transparancy
...
```{r, message=FALSE}
ggplot(data = penguins_data, aes(x = flipper_length_mm, fill = species)) +
  geom_histogram(alpha = 0.99) + #alpha changes transparancy
...
ggplot(data = penguins_data, aes(x = flipper_length_mm, y = body_mass_g , color = species, shape = island))+ 
  geom_point()+ 
  scale_shape_manual(values = c(11,14,8))
...
plotname <- ggplot(data = penguin_data_from_url, aes(x = culmen_depth_mm, fill = species))+
  geom_histogram()+
  theme_bw()+
  scale_fill_viridis_d(option = "plasma", name = "Penguin")
